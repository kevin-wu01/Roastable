...
export function validateToken(token) {
    axios.post("http://localhost:4321/users/validate", null,  authObject(token))
    .then((validation) => {
...
export function getUserData(token) {
    return axios.get("http://localhost:4321/users/get",  authObject(token))
    .then((userData) => {
...
export function getUserConversations(token) {
    return axios.get("http://localhost:4321/conversation/get", authObject(token))
    .then((userData) => {
        return userData.data;
    })
    .catch(err => console.log(err));
